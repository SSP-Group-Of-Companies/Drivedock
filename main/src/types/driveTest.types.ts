...
export const PreTripLabels = {
  ...
  outside: [
    { key: "general_condition_appearance", label: "General Condition/Appearance" },
  ...
  uncoupling: [
    { key: "confirms_stable_ground_for_trailer", label: "Confirms stable ground for trailer" },
    { key: "lowers_landing_gear_correctly", label: "Lowers landing gear correctly" },
    { key: "properly_releases_fifth_wheel", label: "Properly releases FifthWheel" },
    { key: "pulls_out_slowly_from_under_trailer", label: "Pulls out slowly from under trailer" },
    { key: "confirms_trailer_is_properly_supported", label: "Confirms trailer is properly supported" },
    { key: "disconnects_air_electrical_lines_properly", label: "Disconnects Air Electricals lines properly" },
  ] as Array<{ key: PreTripUncouplingKey; label: string }>,

  coupling: [
    { key: "properly_lines_up_units", label: "Properly Lines Up Units" },
    { key: "confirms_trailer_height_before_backing_under", label: "Confirms Trailer height before backing under" },
    { key: "backs_under_slowly", label: "Backs under slowly" },
    { key: "tests_hookup_with_power", label: "Tests Hook-up with Power" },
    { key: "visually_inspects_connection", label: "Visually inspects Connection" },
    { key: "hooks_air_electrical_lines_up_properly", label: "Hooks Air/Electrical Lines up properly" },
    { key: "handles_landing_gear_properly_stows_handles", label: "Handles landing gear properly / stows handles" },
  ...
  backingUp: [
    { key: "approaches_backing_area_slowly", label: "Approaches backing area slowly" },
    { key: "positions_correctly_on_approach_for_the_back", label: "Positions correctly on approach for the back" },
    { key: "uses_4_way_flashers", label: "Uses 4‑Way Flashers" },
    { key: "gets_out_to_inspect_area_prior_to_starting_backup", label: "Gets out to inspect area prior to starting the back‑up" },
    { key: "sounds_horn_before_beginning_to_backup", label: "Sounds horn before beginning to Back‑up" },
    { key: "checks_for_vehicles_or_pedestrians_while_backing", label: "Checks for Vehicles and/or Pedestrians while Backing‑up" },
    { key: "avoids_backing_from_blind_side", label: "Avoids backing from Blind Side" },
    { key: "uses_mirrors_correctly_throughout_backing", label: "Uses mirrors correctly throughout the backing‑up" },
    { key: "steers_correctly_while_backing", label: "Steers correctly while backing‑up" },
    { key: "does_not_oversteer", label: "Does not Oversteer" },
    { key: "pulls_forward_to_reposition_as_required", label: "Pulls forward to reposition as when required" },
    { key: "properly_secures_vehicles_once_parked", label: "Properly secures vehicles once Parked" },
  ] as Array<{ key: PreTripBackingKey; label: string }>,
};

export const OnRoadLabels = {
  inMotion: [
    { key: "selects_correct_gear_for_starting", label: "Selects correct gear for starting" },
    { key: "prevents_roll_back_when_starting", label: "Prevents Roll‑Back when Starting" },
    { key: "up_shifts_correctly", label: "Up shifts Correctly" },
    { key: "uses_clutch_properly", label: "Uses Clutch Properly" },
    { key: "uses_proper_shifting_techniques", label: "Uses Proper Shifting Techniques" },
  ] as Array<{ key: OnRoadInMotionKey; label: string }>,

  highway: [
    { key: "approaches_highway_entrance_properly", label: "Approaches highway entrance properly" },
    { key: "uses_acceleration_ramp_properly", label: "Uses Acceleration Ramp Properly" },
    { key: "selects_correct_gear_in_advance_of_beginning_turn", label: "Selects correct gear in advance of beginning turn" },
    { key: "merges_smoothly_with_traffic", label: "Merges smoothly with traffic" },
    { key: "selects_correct_lane_while_driving_along", label: "Selects correct lane while driving along" },
    { key: "checks_mirrors_regularly_for_following_traffic", label: "Checks mirrors regularly for following traffic" },
    { key: "maintains_safe_distance_between_vehicles", label: "Maintains safe distance between vehicles" },
    { key: "checks_other_traffic_before_beginning_turns", label: "Checks other traffic before beginning turns" },
    { key: "anticipates_actions_of_other_drivers", label: "Anticipates Actions of Other Drivers" },
    { key: "uses_deceleration_ramp_properly", label: "Uses Deceleration Ramp Properly" },
    { key: "exits_highways_safely", label: "Exits Highways Safely" },
    { key: "watches_for_changes_in_traffic_conditions", label: "Watches for changes in Traffic Conditions" },
  ] as Array<{ key: OnRoadHighwayKey; label: string }>,

  turns: [
    { key: "signals_turn_well_in_advance", label: "Signals turn well in advance" },
    { key: "approaches_the_turn_smoothly", label: "Approaches the turn smoothly" },
    { key: "selects_correct_lane_on_approach_to_turn", label: "Selects correct lane on approach to turn" },
    { key: "uses_signal_to_merge_with_traffic", label: "Uses Signal to merge with Traffic" },
    { key: "avoids_shifting_while_turning", label: "Avoids shifting while turning" },
    { key: "turns_into_the_correct_lane", label: "Turns into the correct lane" },
    { key: "uses_mirrors_throughout_the_turn", label: "Uses mirrors throughout the turn" },
  ] as Array<{ key: OnRoadTurnsKey; label: string }>,

  defensive: [
    { key: "understands_all_vehicle_systems", label: "Understands All Vehicle Systems" },
    { key: "understands_obeys_all_warning_and_traffic_signs", label: "Understands / Obeys all warning and traffic signs" },
    { key: "courteous_to_other_drivers", label: "Courteous to Other Drivers" },
    { key: "utilizes_proper_space_management", label: "Utilizes proper space management" },
    { key: "good_traffic_awareness_and_analysis", label: "Good Traffic awareness and analysis" },
    { key: "understands_correct_uses", label: "Understands Correct Uses" },
    { key: "constantly_alert_and_attentive", label: "Constantly Alert and Attentive" },
    { key: "uses_care_when_crossing_railways", label: "Uses care when crossing railways" },
    { key: "maintains_constant_speed", label: "Maintains constant speed" },
    { key: "speed_constant_with_basic_ability", label: "Speed constant with basic ability" },
    { key: "speed_within_legal_limits", label: "Speed within Legal Limits" },
    { key: "does_not_stop_on_railway_tracks", label: "Does not stop on railway tracks" },
    { key: "regularly_checks_instrument", label: "Regularly checks instrument" },
    { key: "properly_inspects_cargo_for_sustainability_securement", label: "Properly inspects cargo for sustainability / Securement" },
    { key: "understands_effects_of_unsecured_cargo", label: "Understands the effects of unsecured cargo" },
    { key: "familiar_with_all_equipment_used_for_securement", label: "Familiar with all equipment used for securement" },
  ] as Array<{ key: OnRoadDefensiveKey; label: string }>,

  gps: [
    { key: "good_traffic_awareness_and_analysis_gps", label: "Good Traffic awareness and analysis" },
    { key: "understands_correct_uses_gps", label: "Understands Correct Uses" },
    { key: "constantly_alert_and_attentive_gps", label: "Constantly Alert and Attentive" },
  ] as Array<{ key: OnRoadGpsKey; label: string }>,

  traffic: [
    { key: "selects_maintains_correct_lane_position", label: "Selects/Maintains Correct Lane Position" },
    { key: "uses_mirrors_correctly_to_check_traffic", label: "Uses Mirrors correctly to check traffic" },
    { key: "uses_proper_eye_lead_time_in_traffic", label: "Uses Proper Eye Lead Time in Traffic" },
    { key: "maintains_safe_distance_from_vehicles", label: "Maintains Safe Distance from Vehicles" },
    { key: "anticipates_actions_of_other_motorists", label: "Anticipates actions of other Motorists" },
    { key: "anticipates_changing_traffic_conditions", label: "Anticipates changing traffic conditions" },
    { key: "observes_properly_on_approach", label: "Observes Properly on Approach" },
    { key: "slows_gears_down_properly_on_approach", label: "Slows / Gears down properly on approach" },
    ...
    { key: "leaves_proper_space_in_front", label: "Leaves Proper Space in Front" },
    { key: "avoids_sudden_hard_stops", label: "Avoids Sudden / Hard Stops" },
    { key: "yields_right_of_way_to_vehicles_pedestrian", label: "Yields Right‑of‑way to vehicles / Pedestrian" },
