  ...
  return (
    <section className="rounded-2xl border border-gray-200 p-6 bg-white shadow-sm">
      ...
      <div className="space-y-6">
        ...
        {/* Daily Hours — Desktop table + Mobile vertical */}
        <div>
          {/* Desktop/tablet */}
          <div className="hidden sm:block">
            <div className="border border-gray-200 rounded-lg bg-gray-50 overflow-x-auto">
              <table className="w-full table-fixed text-center">
                <thead>
                  <tr>
                    <th className="w-20 py-2 text-sm font-semibold text-gray-700 border-b border-gray-200">
                      {t("form.step2.page3.fields.dayHeader", "Day")}
                    </th>
                ...
                <tbody>
                  <tr>
                    <td className="py-2 text-left pl-4 text-sm font-semibold text-gray-800 border-t">
                      {t("form.step2.page3.fields.hoursHeader", "Hours")}
                    </td>
                    {fields.map((field, index) => (
                      <td key={field.id} className="py-3 border-t">
                        ...
                        {errors.canadianHoursOfService?.dailyHours?.[index]?.hours && (
                          <p className="text-red-500 text-xs mt-1">
                            {errors.canadianHoursOfService.dailyHours[index]?.hours?.message}
                          </p>
                        )}
                      </td>
                    ))}
          ...
          <div className="sm:hidden w-full">
            <div className="border border-gray-200 rounded-lg bg-white overflow-hidden w-full">
              <div className="grid grid-cols-[72px_1fr] w-full">
                ...
                <div className="px-3 py-2 text-sm text-center font-semibold text-gray-700 border-b border-black-700 border-l border-black-700">{t("form.step2.page3.fields.hoursHeader", "Hours")}</div>
                {fields.map((field, index) => (
                  <div key={field.id} className="contents ">
                    ...
                    <div className="px-3 py-2 border-l border-black-700 border-b border-gray-500">
                      ...
                      <input
                        ...
                        max={24}
                        step={0.25}
                        className="w-full text-center bg-white border border-0 rounded-md  focus:ring-sky-500 focus:outline-none focus:shadow-md text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                        placeholder="0"
                        data-field={`canadianHoursOfService.dailyHours.${index}.hours`}
                        {...register(
                          `canadianHoursOfService.dailyHours.${index}.hours` as const,
