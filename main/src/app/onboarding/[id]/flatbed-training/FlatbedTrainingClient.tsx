...
export type FlatbedTrainingClientProps = {
...
export default function FlatbedTrainingClient({
  ...
  const completed = flatbedTraining.completed ?? false;
  const [showConfetti, setShowConfetti] = useState(false);

  // Show confetti when component mounts and flatbed training is completed
  useEffect(() => {
    if (completed) {
        ...
        >
          ...
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 max-w-2xl mx-auto">
            <h2 className="text-lg font-bold text-gray-800 pb-2">
              {t("form.step7.success.title")}
            </h2>
            <p className="text-sm text-green-800 max-w-2xl mx-auto">
              {t("form.step7.success.message")}
            </p>
          </div>
          ...
          >
            {t("form.step7.success.continueButton")}
          </button>
        </motion.div>
      );
    }
    ...
    return (
      ...
      >
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 max-w-3xl mx-auto text-left space-y-4">
            <p className="text-gray-700 leading-relaxed text-sm text-center font-semibold">
              {t("form.step7.instructions.header")}
            </p>

            <div className="flex items-start gap-3">
              <Users className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
              <p className="text-gray-700 text-sm">
                {t("form.step7.instructions.contactSafety")}
              </p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <p className="text-gray-700 text-sm">
                  {t("form.step7.instructions.passMessage")}
                </p>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="text-gray-700 text-sm">
                  {t("form.step7.instructions.failMessage")}
                </p>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                {t("form.step7.instructions.note")}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="pt-4">
          ...
          >
            {t("form.step7.buttons.completeTraining")}
          </button>
        </div>
      </motion.div>
    );
