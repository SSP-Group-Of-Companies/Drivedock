...
/**
 * DriveDock Onboarding — Drive Test
 * Server Wrapper
 *
 * Responsibilities
 * - Fetch saved Drive Test data + onboardingContext by tracker ID
...
export default async function OnboardingDriveTestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const url = `${base}/api/v1/onboarding/${trackerId}/drive-test`;

  // Unified fetch pattern (handles cookies/redirects/JSON + unwraps { data })
  const { data, error } = await fetchServerPageData<DriveTestClientProps>(url);

  if (error) {
    return <div className="p-6 text-center text-red-600 font-semibold">{error}</div>;
  }

  if (!data?.driveTest || !data?.onboardingContext) {
    return <div className="p-6 text-center text-red-600 font-semibold">Failed to load data. Please try again later.</div>;
  }

  return <DriveTestClient driveTest={data.driveTest} onboardingContext={data.onboardingContext} />;
}
