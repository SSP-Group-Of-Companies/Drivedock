...
function isBusinessClearIntent(b: PatchBody) {
  ...
  const emptyStrings =
    (!b.employeeNumber || b.employeeNumber.trim() === "") &&
    (!b.businessNumber || b.businessNumber.trim() === "") &&
    (!b.hstNumber || b.hstNumber.trim() === "");

  const emptyPhotos =
    alen(b.incorporatePhotos) === 0 &&
    alen(b.bankingInfoPhotos) === 0 &&
    alen(b.hstPhotos) === 0;
  ...
  try {
    ...
    // (B) Page 4 — explicit, body-driven
    {
      ...
      const nextP4: IApplicationFormPage4 = {
        ...
        ...(hasKey(body, "employeeNumber")
          ? { employeeNumber: body.employeeNumber ?? "" }
          : {}),
        ...(hasKey(body, "businessNumber")
    ...
    if (p4Final.fastCard?.fastCardFrontPhoto) {
      p4Final.fastCard.fastCardFrontPhoto = await finalizePhoto(
    ...
    if (p4Final.fastCard?.fastCardBackPhoto) {
      p4Final.fastCard.fastCardBackPhoto = await finalizePhoto(
