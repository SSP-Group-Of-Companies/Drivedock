...
export async function PATCH(
  ...
  try {
    ...
    if (!hasCompletedStep(onboardingDoc, EStepPath.APPLICATION_PAGE_1))
    ...
    if (!page1)
      return errorResponse(400, "Missing 'personalDetails' in request body");

    // ---- Early MIME-type guard for SIN + license photos (no DB hit yet) ----
    const sinPhotoError = validateImageOrPdfField(
      page1.sinPhoto,
      "personalDetails.sinPhoto",
    );
    if (sinPhotoError) {
    ...
    if (Array.isArray(page1.licenses)) {
      for (let i = 0; i < page1.licenses.length; i++) {
        ...
        const frontErr = validateImageOrPdfField(
          lic?.licenseFrontPhoto,
          `personalDetails.licenses.${i}.licenseFrontPhoto`,
        );
        if (frontErr) {
          return errorResponse(400, frontErr);
        }

        const backErr = validateImageOrPdfField(
          lic?.licenseBackPhoto,
          `personalDetails.licenses.${i}.licenseBackPhoto`,
        );
        if (backErr) {
    ...
    if (!appFormDoc.page1)
      return errorResponse(401, "driver hasn't filled in personal details yet");
    ...
    if (
      originalSinPhoto?.s3Key &&
      workingSinPhoto?.s3Key &&
      originalSinPhoto.s3Key !== workingSinPhoto.s3Key
    ) {
    ...
    await recordOnboardingAuditLogSafe({
      onboardingId,
      action: EOnboardingAuditAction.PERSONAL_DETAILS_UPDATED,
      actor: actorFromAdminUser(adminUser),
      message: sinChanged
        ? "Administrator updated personal details (page 1), including identity values tied to the SIN."
        : "Administrator updated personal details (page 1), including name, contact, or license image fields.",
      metadata: {
        sinChanged: Boolean(sinChanged),
        page: "page-1",
      },
    });
