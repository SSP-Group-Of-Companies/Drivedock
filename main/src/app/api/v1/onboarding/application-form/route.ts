...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!page1.sinPhoto || !page1.sinPhoto.mimeType)
      return errorResponse(
        400,
        "SIN document is required and must be a PDF file",
      );
    ...
    for (let i = 0; i < licenses.length; i++) {
      ...
      if (lic.licenseFrontPhoto) {
        const mimeFront = String(
          lic.licenseFrontPhoto.mimeType || "",
        ).toLowerCase();
        if (mimeFront !== EFileMimeType.PDF) {
          return errorResponse(
            400,
            `License #${idx} front document must be a PDF file`,
          );
        }
      }

      if (lic.licenseBackPhoto) {
        const mimeBack = String(
          lic.licenseBackPhoto.mimeType || "",
        ).toLowerCase();
        if (mimeBack !== EFileMimeType.PDF) {
          return errorResponse(
            400,
            `License #${idx} back document must be a PDF file`,
          );
        }
      }
    }
    ...
    await recordOnboardingAuditLogSafe({
      onboardingId: String(onboardingDoc._id),
      action: EOnboardingAuditAction.ONBOARDING_APPLICATION_SUBMITTED,
      actor: actorFromDriverPage1(page1, String(onboardingDoc._id)),
      message:
        "Driver submitted a new digital onboarding application (personal details on page 1 and prequalification answers). The application is pending company approval.",
      metadata: {
        countryCode,
        invitationApproved: false,
        driverEmail: page1.email?.trim() || undefined,
        prequalificationFieldsSubmitted: Object.keys(
          prequalifications ?? {},
        ).filter(
          (k) =>
            (prequalifications as Record<string, unknown>)[k] !== undefined,
        ),
      },
    });
