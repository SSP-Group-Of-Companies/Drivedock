...
import { onboardingExpired, buildTrackerContext, advanceStatus } from "@/lib/utils/onboardingUtils";
...
export async function PATCH(
  ...
  try {
    ...
    if (!Array.isArray(page1.licenses)) return errorResponse(400, "'licenses' must be an array");
    ...
    if (!updatedForm) return errorResponse(500, "Failed to update ApplicationForm");
    ...
    if (!finalSave) return errorResponse(500, "Failed to update finalized photos");
