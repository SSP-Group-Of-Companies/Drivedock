...
function PreTripSignatureField({ sigRef, trackerId, s3Folder, isLocked, initialSignature, onSignatureDirtyChange }: Props) {
  const {
    setValue,
    clearErrors,
    formState: { errors },
  ...
  const sigS3Err = errors?.preTrip?.supervisorSignature?.s3Key?.message as string | undefined;
  const sigUrlErr = errors?.preTrip?.supervisorSignature?.url?.message as string | undefined;
...
export default memo(PreTripSignatureField);
