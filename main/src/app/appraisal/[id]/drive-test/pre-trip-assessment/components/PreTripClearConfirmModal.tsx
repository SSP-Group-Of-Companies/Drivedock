...
export default function PreTripClearConfirmModal({ open, onCancel, onConfirm }: Props) {
  return (
    <AnimatePresence>
      {open && (
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Dialog.Panel className="w-full max-w-md rounded-lg shadow-xl overflow-hidden bg-white">
              <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }} transition={{ duration: 0.2 }}>
                <div className="p-6">
                  <DialogTitle className="text-lg font-bold text-gray-800 mb-4">Clear Pre-Trip Assessment?</DialogTitle>
                  <Description className="text-sm text-gray-600 mb-6">
                    This will reset all fields in this pre-trip assessment, including the signature. This action cannot be undone. Are you sure you want to continue?
                  </Description>
