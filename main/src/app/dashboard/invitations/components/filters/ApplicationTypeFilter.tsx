...
export default function ApplicationTypeFilter({ options, selected, onChange }: Props) {
  const selectedSet = useMemo(() => new Set(selected ?? []), [selected]);
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement | null>(null);
  const btnId = "invitations-app-type-filter-button";
  const listId = "invitations-app-type-filter-list";
  ...
  const label = selectedSet.size ? `${selectedSet.size} selected` : "Select app type";
