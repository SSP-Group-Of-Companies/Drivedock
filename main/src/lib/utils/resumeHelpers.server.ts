...
export async function getTrackerAndDriverEmailBySinHash(sinHash: string) {
  const tracker = await OnboardingTracker.findOne({ sinHash }).lean();
  ...
  if (appFormId) {
    const appForm = await ApplicationForm.findById(appFormId).lean<IApplicationFormDoc | null>();
    const page1 = appForm?.page1 as unknown as IApplicationFormPage1 | undefined;
    if (page1) {
